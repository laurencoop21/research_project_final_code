...
model_path = MODEL_DIR / "checkpoints" / "model_15_best.pt"
...
tile_results.to_csv(
    reports_dir / "model_15_error_analysis_by_tile.csv",
    index=False,
)

prediction_results_table.to_csv(
    reports_dir / "model_15_test_predictions.csv",
    index=False,
)
...
ground_truth_results.to_csv(
    reports_dir / "model_15_test_ground_truth_matches.csv",
    index=False,
)
...
size_summary.to_csv(
    reports_dir / "model_15_recall_by_box_size.csv",
    index=False,
)
...
summary_path = (
    reports_dir / "model_15_error_analysis_summary.json"
)

with open(summary_path, "w") as file:
    json.dump(summary, file, indent=4)

print("\nModel 15 test error analysis")
print("---------------------------")
